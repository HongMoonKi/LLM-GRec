...
class llm4rec(nn.Module):
    ...
    def rec_loss(self, anchor, items):
        logits = torch.bmm(items.view(anchor.shape[0], -1, anchor.shape[1]), 
                        anchor.unsqueeze(2)).squeeze(2)
        labels = torch.zeros(logits.size(0), dtype=torch.long).to(logits.device)
        
        # ❌ 기존: loss = F.cross_entropy(logits, labels)
        # ✅ 수정: reduction='none'으로 샘플별 loss 반환
    ...
    def train_mode0(self,samples):
        ...
        if hasattr(self, 'args') and hasattr(self.args, 'distill_loss') and self.args.distill_loss == 'cosine':
            # Cosine similarity loss
            cos_loss = (1 - F.cosine_similarity(user_outputs, log_emb, dim=-1)).mean()
            mse_loss = self.mse(user_outputs, log_emb)
            match_loss = 0.3 * cos_loss + 0.7 * mse_loss
        else:
            # MSE loss (기존)
        ...
        loss = rec_loss.mean() + match_loss
        
        return None, rec_loss, match_loss.item()